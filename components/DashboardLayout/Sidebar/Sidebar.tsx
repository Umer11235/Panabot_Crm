...
const SidebarV1 = ({ isMobileOpen = false, onCloseMobile }: SidebarProps) => {
  ...
  return (
    <>
      {isMobileOpen && <div className={styles.mobileOverlay} onClick={onCloseMobile} />}
      <aside
      ...
      >
        ...
        <div className={styles.menuContainer}>
          <div className="mb-2">
            <Link
              href="/dashboard"
              className={`${styles.menuItem} ${pathname === '/dashboard' ? styles.activeSub : ''}`}
              ...
              {section.items.map((item) => (
                <div key={item.title} className="mb-1">
                  {!item.children ? (
                    <Link
                      href={item.path || '#'}
                      className={`${styles.menuItem} ${pathname === item.path ? styles.activeSub : ''}`}
                      onClick={onCloseMobile}
                    >
                      <div className="flex items-center gap-3">
